...
def add_exercise_evaluation_for_member(
    ...
    """
    exercise for scheduling the inference with 1. evaluation of leaf nodes, 2. evaluation of sum and product nodes,
    3. joining the root values
    :param manager: manager in the network
    :param member_id: member to send exercise to
    :param line_index_of_input_for_private_evaluation: data point to be inferred
    """
...
def evaluation_single_member_leaf_step(member, message_value):
    ...
    if single_member_id == member.id:
        ...
        print(private_input_for_leafes_current_line)
        private_input_for_leafes = np.array([private_input_for_leafes_current_line]).reshape(-1, len(private_input_for_leafes_current_line))
        print(private_input_for_leafes)
        for index in range(len(leaf_nodes)):
            ...
            data_id_leaf_node = f"{structure_id}_({leaf_node.id}, {leaf_node.id})_p_result"
            if private_input_for_leafes_current_line[leaf_node.scope[0]] == 1: 
                result_value_for_leaf = 1
            else: 
                result_value_for_leaf = 0
            #print("exact result {}".format(exact_result[0]))
            #print( data_id_leaf_node, result_value_for_leaf)
            member.insert_in_share(
                data_id_leaf_node, result_value_for_leaf, with_id_appendix=False
            )
            
            data_id_leaf_node = f"{structure_id}_({leaf_node.id}, {leaf_node.id})_(1-p)_result"
            if private_input_for_leafes_current_line[leaf_node.scope[0]] == 1: 
                result_value_for_leaf = 0
            else: 
                result_value_for_leaf = 1
...
def evaluation_single_member_join_step(member, message_value):
    ...
    for i in range(len(member.spn)):
        if single_member_id == member.id:
            ...
            print(
                f"For member {single_member_id} the {data_id_result}_for_{line_index_of_input_for_private_evaluation} is {member.data.get(data_id_result)/ (member.d_multiplyer**3)}"
            )

    
    if single_member_id == member.id:
        member.data[data_id_to_save] = member.data[data_id_to_save] / (member.d_multiplyer**3)
        with open(f"/ppspn_env/resources/output/{single_member_id}/structure_0.out", "a") as out_file:
