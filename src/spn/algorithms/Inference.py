...
def leaf_marginalized_likelihood(
    node, data=None, dtype=np.float64, log_space=False, **kwargs
):
...
def sum_likelihood_original(node, children, dtype=np.float64, **kwargs):
    llchildren = np.concatenate(children, axis=1)
...
def sum_likelihood(node, children, dtype=np.float64, **kwargs):
    llchildren = np.concatenate(children, axis=1)
    insert_edge_usage = kwargs.get("insert_edge_usage")
    if insert_edge_usage is not None and insert_edge_usage:
        member = kwargs.get("member")
        #### inserted ####
        for index in range(len(llchildren)):
            currentValues = llchildren[index]
            # childIndexOfGreaterZero = np.where(currentValues > 0)[0][0]
            for index_of_child_with_value_greater_zero in np.where(currentValues > 0)[
                0
            ]:
                childProductNode = node.children[index_of_child_with_value_greater_zero]
                currentAmount = member.used_edges.get((node.id, childProductNode.id))
                if currentAmount == None:
                    currentAmount = 0
                currentAmount += 1
                member.used_edges[(node.id, childProductNode.id)] = currentAmount
        ##### ####
    ...
    **kwargs,
    ...
    if debug:
        ...
        def exec_funct(node, *args, **kwargs):
            ...
            assert ll.shape == (data.shape[0], 1), (
...
def log_likelihood(
    ...
    **kwargs,
    ...
    return likelihood(
        ...
        data,
        dtype=dtype,
        node_likelihood=node_log_likelihood,
...
def conditional_log_likelihood(
    node_joint, node_marginal, data, log_space=True, dtype=np.float64
):
    result = log_likelihood(node_joint, data, dtype) - log_likelihood(
        node_marginal, data, dtype
    )
    if log_space:
        return result
