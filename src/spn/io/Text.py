...
def spn_to_str_ref_graph(node, feature_names=None, node_to_str=None):
    ...
    if isinstance(node, Product):
        ...
        chld_str = "".join(
            map(
                lambda c: spn_to_str_ref_graph(c, feature_names, node_to_str),
                node.children,
            )
        )
...
def spn_to_str_equation(node, feature_names=None, node_to_str=_node_to_str):
    ...
    if isinstance(node, Product):
        children_strs = map(
            lambda child: spn_to_str_equation(child, feature_names, node_to_str),
            node.children,
        )
    ...
    if isinstance(node, Sum):
        ...
        def fmt_chld(w, c):
            return (
                str(w) + "*(" + spn_to_str_equation(c, feature_names, node_to_str) + ")"
            )

        children_strs = map(
            lambda i: fmt_chld(node.weights[i], node.children[i]),
...
"""
    ...
    def tree_to_spn(tree, features):
        ...
        if tnode in str_to_spn_lambdas:
            return str_to_spn_lambdas[tnode][0](
                tree, features, str_to_spn_lambdas[tnode][2], tree_to_spn
            )
