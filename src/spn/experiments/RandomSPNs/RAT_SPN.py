...
import tensorflow_probability.python.distributions as dists
...
def print_if_nan(tensor, msg):
    ...
    return tf.cond(
        is_nan,
        lambda: tf.Print(tensor, [is_nan], message=msg),
...
class GaussVector(NodeVector):
    def __init__(
        self, region, args, name, given_means=None, given_stddevs=None, mean=0.0
    ):
        super().__init__(name)
        ...
        if args.gauss_min_var < args.gauss_max_var:
            ...
            self.sigma = args.gauss_min_var + (
                args.gauss_max_var - args.gauss_min_var
            ) * tf.sigmoid(self.sigma_params)
        else:
            self.sigma = 1.0
    ...
    def forward(self, inputs, marginalized=None):
        ...
        if marginalized is not None:
            ...
            local_marginalized = tf.expand_dims(
                tf.gather(marginalized, self.scope, axis=1), axis=-1
            )
    ...
    def sample(self, num_samples, num_dims, seed=None):
        ...
        sample_values = self.means + tf.zeros(
            [num_samples, self.local_size, self.args.num_univ_distros]
        )
...
class BernoulliVector(NodeVector):
    ...
    def forward(self, inputs, marginalized=None, classes=False):
        local_inputs = tf.gather(inputs, self.scope, axis=1)
        bernoulli_log_pdf_single = self.dist.log_prob(
        ...
        if marginalized is not None:
            ...
            bernoulli_log_pdf_single = bernoulli_log_pdf_single * (
                1 - tf.cast(local_marginalized, dtype=tf.float32)
            )
    ...
    def sample(self, num_samples, num_dims, seed=None):
        sample_values = self.probs + tf.zeros(
            [num_samples, self.local_size, self.args.num_univ_distros]
        )
        sample_shape = [num_samples, num_dims, self.size]
        indices = tf.meshgrid(tf.range(num_samples), self.scope, tf.range(self.size))
...
class ProductVector(NodeVector):
    ...
    def sample(self, inputs, seed=None):
        ...
        output_shape = [
            inputs[0].shape[0],
            inputs[0].shape[1],
            (inputs[0].shape[2] * inputs[1].shape[2]),
        ]
...
class SumVector(NodeVector):
    def __init__(
        self, prod_vectors, num_sums, args, dropout_op=None, name="", given_weights=None
    ):
        super().__init__(name)
            ...
            if args.normalized_sums:
                ...
                if args.sum_weight_l2:
                    ...
                    weight_decay = tf.multiply(
                        tf.nn.l2_loss(exp_weights), args.sum_weight_l2
                    )
                    tf.add_to_collection("losses", weight_decay)
                    tf.add_to_collection("weight_losses", weight_decay)
            else:
            ...
            if self.dropout_op is not None:
                if self.args.drop_connect:
                    ...
                    random_tensor = random_ops.random_uniform(
                        dropout_shape, dtype=self.weights.dtype
                    )
                    dropout_mask = tf.log(
                        math_ops.floor(self.dropout_op + random_tensor)
                    )
                    weights = weights + dropout_mask

                else:
                    random_tensor = random_ops.random_uniform(
                        prods.shape, dtype=prods.dtype
                    )
                    dropout_mask = tf.log(
                        math_ops.floor(self.dropout_op + random_tensor)
                    )
    ...
    def sample(self, inputs, seed=None):
        ...
        indices = tf.reshape(
            tf.tile(indices, [1, inputs.shape[1]]),
            [inputs.shape[0], self.size, inputs.shape[1]],
        )
        indices = tf.transpose(indices, [0, 2, 1])

        others = tf.meshgrid(
            tf.range(inputs.shape[1]), tf.range(inputs.shape[0]), tf.range(self.size)
        )
...
class RatSpn(object):
                ...
                for k, a_node in enumerate(prod_node.children):
                    ...
                    if self.args.leaf == "bernoulli":
                        ...
                        bernoulli_vector = BernoulliVector(
                            ...
                            self.args,
                            name,
                            given_params=a_node.probs.eval(session=sess),
                        )
                        init_new_vars_op = tf.initializers.variables(
                            [bernoulli_vector.probs], name="init"
                        )
            ...
            for vector_num, vector in enumerate(layer):
                if type(vector[0]) == base.Product:
                    ...
                    child_vecs = list(
                        set(
                            [
                                node_to_vec[id(child_node)]
                                for child_node in vector[0].children
                            ]
                        )
                    )
                    ...
                    new_vector = SumVector(
                        child_vecs,
                        len(vector),
                        self.args,
                        name=name,
                        given_weights=weights,
                    )
                else:
                    assert False
    ...
    def _make_spn_from_region_graph(self):
        ...
        for i, leaf_region in enumerate(rg_layers[0]):
            if self.args.leaf == "bernoulli":
                ...
                bernoulli_vector = BernoulliVector(
                    leaf_region, self.args, name, p=self.default_param
                )
                ...
                gauss_vector = GaussVector(
                    leaf_region, self.args, name, mean=self.default_mean
                )
        ...
        for layer_idx in range(1, len(rg_layers)):
            ...
            if layer_idx % 2 == 1:
                ...
                for i, region in enumerate(regions):
                    ...
                    sum_vector = SumVector(
                        product_vectors, cur_num_sums, self.args, name=vector_name
                    )
        ...
        self.output_vector = self._region_distributions[
            self._region_graph.get_root_region()
        ]
    ...
    def sample(self, num_samples=10, seed=None):
        vec_to_samples = {}
        for leaf_vector in self.vector_list[0]:
            vec_to_samples[leaf_vector] = leaf_vector.sample(
                num_samples, self.num_dims, seed=seed
            )
            ...
            for i in range(leaf_vector.size):
                ...
                for j, r in enumerate(leaf_vector.scope):
                    if self.args.leaf == "bernoulli":
                        ...
                        stdevs = np.sqrt(sigmas) + np.zeros_like(
                            means
                        )  # Use broadcasting to expand stdev is necessary
                        gaussian = para.Gaussian(
                            mean=means[j, i], stdev=stdevs[j, i], scope=[r]
                        )
            ...
            if layer_idx % 2 == 1:
                ...
                for i, sum_vector in enumerate(sum_vectors):
                    ...
                    for j in range(sum_vector.size):
                        ...
                        input_vecs = [
                            vec_to_nodes[prod_vec] for prod_vec in sum_vector.inputs
                        ]
